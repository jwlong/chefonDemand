...
class FoodItemService extends BaseService{
    ...
    getChefMenuFoodItemsByChefId(chef_id) {
        ...
        return this.baseFindByFilter(fields,{chef_id:chef_id,active_ind:activeIndStatus.ACTIVE}).then( foodItems => {
            let result = {};
    ...
    addChefMenuFoodItem(attr) {
        return db.transaction(t=> {
            return this.getModel().count({where:{chef_id:attr.chef_id,food_item_name:attr.food_item_name,active_ind:activeIndStatus.ACTIVE},transaction:t}).then( itemCnt => {
                if (itemCnt >0) {
    ...
    editChefMenuFoodItem(attr) {
        return db.transaction(t => {
            return this.getModel().count({where:{chef_id:attr.chef_id,food_item_name:attr.food_item_name,active_ind:activeIndStatus.ACTIVE},transaction:t}).then( nameCnt => {
                if (nameCnt >0) {
                    throw baseResult.MENU_FOOD_ITEM_NAME_EXIST;
                }
                return this.getOne({where:{chef_id:attr.chef_id,food_item_id:attr.food_item_id,active_ind:activeIndStatus.ACTIVE}}).then(foodItem => {
                    if (foodItem) {
    ...
    removeChefMenuFoodItem(attr) {
         return this.getOne({where:{food_item_id:attr.food_item_id,active_ind:activeIndStatus.ACTIVE}}).then(foodItem => {
            if (foodItem) {
                if (foodItem.chef_id !== attr.chef_id) {
                    throw baseResult.MENU_FOOD_ITEM_ID_NOT_BELONG_TO_CHEF;
                }
                return this.baseUpdate({active_ind:activeIndStatus.INACTIVE},{where:{food_item_id:attr.food_item_id,chef_id:attr.chef_id}});
